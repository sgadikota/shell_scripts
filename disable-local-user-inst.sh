...
do
  case ${OPTION} in
    d) DELETE_USER='true' ;;
    r) REMOVE_OPTION='-r' ;; # if this option is not supplied, then this evaluates to empty string, if someone specifies -r option, it will deletes home directory.
    a) ARCHIVE='true' ;;
    ?) usage ;;
  esac
done
...
do
  ...
  then
    ...
    then
      ...
      tar -zcf ${ARCHIVE_FILE} ${HOME_DIR} &> /dev/null # -z to compress; -c to create; f is the location of the archive file.
      if [[ "${?}" -ne 0 ]]
      then
  ...
  then
    ...
    # ***** we can write a function that checks the exit status and returns a comment with exit status as 1.
    if  [[ "${?}" -ne 0 ]]
    then
